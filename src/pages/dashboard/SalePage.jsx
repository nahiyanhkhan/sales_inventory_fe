...
const SalePage = () => {
    ...
    useEffect(() => {
        const productTable = $(productTableRef.current).DataTable(dtconfig);
        const customerTable = $(customerTableRef.current).DataTable(dtconfig);
    ...
    if (isLoading) {
        return (
            <>
                <div id="loader" className="LoadingOverlay d-none">
                        ...
                        <div className="shadow-sm h-100 bg-white rounded-3 p-3">
                            <div className="row">
                                ...
                                <div className="col-4">
                                    ...
                                    <p className="text-xs mx-0 my-1">Date: {new Date().toLocaleDateString('en-US')} </p>
                                </div>
                            </div>
                                        ...
                                        <tbody className="w-100" id="invoiceList">
                                            {
                                                invoiceProducts.map((product, index) => {
                                                    <tr className="text-xs" key={product.id}>
                                                        ...
                                                        <td>{product.total}</td>
                                                        <td>
                                                            <a className="btn remove text-xxs px-2 py-1  btn-sm m-0">Remove</a>
                                                        </td>
                                                    </tr>
                                                })
                                            }
                            ...
                            <div className="row">
                                <div className="col-12">
                                    ...
                                    <input onKeyDown="return false" defaultValue="0" min="0" type="number" step="0.25" onChange="DiscountChange()" className="form-control w-40 " id="discountP" />
                                    <p>
                        ...
                        <div className="shadow-sm h-100 bg-white rounded-3 p-3">
                            <div id="productTable_wrapper" className="dataTables_wrapper no-footer">
                                <div id="productTable_filter" className="dataTables_filter">
                                    <label>Search:<input type="search" className="" placeholder="" aria-controls="productTable" /></label>
                                </div>
                                    ...
                                    <tbody className="w-100" id="productList">
                                        {
                                            products.map((product, index) => {
                                                <tr className={`text-xs ${index % 2 == 0 ? "odd" : "even"}`} key={product.id}>
                                                    <td className="sorting_1">
                                                        <img className="w-10" src={product.img_url} /> {product.name} ($ {product.price})
                                                    </td>
                                                    <td>
                                                        <a data-name={product.name} data-price={product.price} data-id={product.id} className="btn btn-outline-dark text-xxs px-2 py-1 addProduct btn-sm m-0">Add</a>
                                                    </td>
                                                </tr>
                                            })
                                        }
                                        {/* <tr className="text-xs odd">
                                            <td className="sorting_1">
                                                <img className="w-10" src="/uploads/1-1692093134-2.jpg" /> sd ($ 52)
                                            </td>
                                ...
                                <div className="dataTables_paginate paging_simple_numbers" id="productTable_paginate">
                                    <a className="paginate_button previous disabled" aria-controls="productTable" aria-disabled="true" role="link" data-dt-idx="previous" tabIndex="-1" id="productTable_previous">Previous</a>
                                    <span><a className="paginate_button current" aria-controls="productTable" role="link" aria-current="page" data-dt-idx="0" tabIndex="0">1</a></span>
                        ...
                        <div className="shadow-sm h-100 bg-white rounded-3 p-3">
                            <div id="customerTable_wrapper" className="dataTables_wrapper no-footer">
                                <div id="customerTable_filter" className="dataTables_filter">
                                    <label>Search:<input type="search" className="" placeholder="" aria-controls="customerTable" /></label>
                                </div>
                                ...
                                <table className="table table-sm w-100 no-footer dataTable" id="customerTable" aria-describedby="customerTable_info" style={{ width: "505px" }} ref={customerTableRef}>
                                    <thead className="w-100">
                                        <tr className="text-xs text-bold"><td className="sorting sorting_desc" tabIndex="0" aria-controls="customerTable" rowSpan="1" colSpan="1" aria-label="Customer: activate to sort column ascending" style={{ width: "274px" }} aria-sort="descending">Customer</td><td className="sorting" tabIndex="0" aria-controls="customerTable" rowSpan="1" colSpan="1" aria-label="Pick: activate to sort column ascending" style={{ width: "159px" }}>Pick</td></tr>
                                    </thead>
                                    <tbody className="w-100" id="customerList">
                                        {
                                            customers.map((customer, index) => {
                                                <tr className={`text-xs ${index % 2 == 0 ? "odd" : "even"}`} key={customer.id}>
                                                    <td className="sorting_1"><i className="bi bi-person"></i> {customer.name}</td>
                                                    <td>
                                                        <a data-name={customer.name} data-email={customer.email} data-id={customer.id} className="btn btn-outline-dark addCustomer text-xxs px-2 py-1 btn-sm m-0">Add</a>
                                                    </td>
                                                </tr>
                                            })
                                        }
                                        {/* <tr className="text-xs odd">
                                            <td className="sorting_1"><i className="bi bi-person"></i> IRS IT</td>
                                            <td><a data-name="IRS IT" data-email="sdsd@sd.com" data-id="1" className="btn btn-outline-dark addCustomer  text-xxs px-2 py-1  btn-sm m-0">Add</a></td>
                                        </tr> */}
                                    </tbody>
                                </table>

                                {/* <div className="dataTables_paginate paging_simple_numbers" id="customerTable_paginate">
                                    <a className="paginate_button previous disabled" aria-controls="customerTable" aria-disabled="true" role="link" data-dt-idx="previous" tabIndex="-1" id="customerTable_previous">Previous</a>
                                    <span><a className="paginate_button current" aria-controls="customerTable" role="link" aria-current="page" data-dt-idx="0" tabIndex="0">1</a></span>
                                    <a className="paginate_button next disabled" aria-controls="customerTable" aria-disabled="true" role="link" data-dt-idx="next" tabIndex="-1" id="customerTable_next">Next</a>
                                </div> */}
                            </div>
                        </div>
                    </div>
